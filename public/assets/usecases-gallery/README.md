...
The homepage embeds this same-origin snapshot at 800px height, as the Webflow prototype does. It is deliberately isolated from the marketing site's CSS. Case JSON, diagrams, images and files are local; external product/share links retain their original destinations. Files marked as placeholders in the source remain explicitly marked, not fabricated.

Local adaptations: local open-source fonts; cancel obsolete fade timers; image dialog focus restoration/trapping and scroll locking; language persistence only after locale consent; noindex for this embedded document. The marketing CMS schemas, published documents, pricing, authentication and remote data are not changed by this snapshot.
