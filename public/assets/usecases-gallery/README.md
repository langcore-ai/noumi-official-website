...
Local adaptations: local open-source fonts; cancel obsolete fade timers; image dialog focus restoration/trapping and scroll locking; trackpad/touch swiping on the media row (native `overflow-x` scrolling with the scrollbar hidden, arrows stay in sync with the reader's position); English-only display without a language switch or stored-language preference; noindex for this embedded document. The marketing CMS schemas, published documents, pricing, authentication and remote data are not changed by this snapshot.
